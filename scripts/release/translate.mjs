...
export const translate = async (text, source, target) => {
    ...
    if (provider === LinguineProvider.GOOGLE) {
        ...
        if (target === 'zh-Hant')
            target = 'zh-TW';
        text = await Promise.all(text.map(async (t, idx) => {
            await new Promise((resolve) => setTimeout(resolve, 1000 * idx));
            const result = await fetch(`https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&from=${source}&to=${target}`, {
                method: 'POST',
                headers: {
        ...
        if (target === 'zh-CN' || target === 'zh-Hans')
            target = 'zh';
        if (target === 'pt')
            target = 'pt-BR';
        if (target === 'en')
            target = 'en-US';
        const sourceLanguageCode = source || 'en';
        const targetLanguageCode = target || 'en-US';
