...
const main = async () => {
    ...
    for (let i = 0; i < LinguineList.length; i++) {
        ...
        localeStories[locale] = await Promise.all(stories.map(async (s) => {
    ...
    for (let i = 0; i < LinguineList.length; i++) {
        ...
        if (locale === 'en') {
            ...
            fs.writeFileSync(`./i18n/${locale}/docusaurus-plugin-content-docs/current/${day.replaceAll('-', '/')}.md`, createContent(locale, localeStories[locale]));
        }
    }
