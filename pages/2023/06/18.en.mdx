...
- The London TFL Dot Matrix Typeface is a collection of fonts that mimic the style of the fonts used in the London Underground's arrival boards and announcement boards.
...
- A user reminisces about creating their own video game engine in the 90s and manually copying fonts from Lucasfilm games, similar to the London Underground Dot Matrix Typeface.
...
- This post is an investigation into the symbol U+237C ⍼ RIGHT ANGLE WITH DOWNWARDS ZIGZAG ARROW.
...
- The website http://xml.coverpages.org hosts pre-2000 material related to ISO 8879 (SGML) and XML.
...
- The phenomenon of "bullshit jobs" refers to forms of employment that are considered pointless, unnecessary, or even pernicious by the person doing the job.
...
- The book "Bullshit Jobs" explores the concept of jobs that are seen as pointless, unnecessary, or pernicious by the employees themselves.
...
- Every year, new general-purpose database engines are released with new query languages.
...
- SQL has limitations when it comes to querying heterogeneous databases and dealing with multiplicity mismatch in JOINs.
...
- GB Studio is a drag and drop retro game creator for the GameBoy handheld video game system.
...
- GB Studio is a game engine that allows users to create retro games for the GameBoy
...
- Research shows that suspects can be convinced they committed crimes that they didn't actually commit
...
- Researchers have found that false memories can be implanted, leading people to believe they committed a crime that never happened.
...
- Users are questioning why Apple has not added a feature called "window snapping" to their macOS operating system.
...
- Users are frustrated with the absence of a built-in window snapping feature in macOS.
...
- Large Language Models (LLMs) can now process an extremely large context window, up to 100K input tokens, during training and inference.
...
- The article discusses the techniques used to extend the context window in LLMs and make them work effectively.
...
- Hetzner's ARM64 servers perform well in webP conversion speed, only 8% slower than their AMD64 counterparts, with a 14% price difference and double the amount of RAM.
...
- The article discusses the review of Hetzner ARM64 servers and the experience of using WebP cloud services on them.
...
- The bcachefs filesystem was discussed at the 2023 Linux Storage, Filesystem, Memory-Management and BPF Summit, and the process for merging it into the mainline kernel seems closer than ever.
...
- Bcachefs is a file system that is in a good position to be merged into the Linux kernel.
